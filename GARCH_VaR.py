...
# Calculate simple standard deviation of log returns
simple_vol = df['log_returns'].std()
...
print(
    f'Ljung-Box Q test for lag 10 -- statistic: {lb_test["lb_stat"].values[0]}, p-value: {lb_test["lb_pvalue"].values[0]}'
)
...
print(
    f'ARCH test -- LM statistic: {arch_test[0]}, LM-Test p-value: {arch_test[1]}, F-statistic: {arch_test[2]}, F-Test p-value: {arch_test[3]}'
)
...
# VaR - simple_vol
daily_VaR_simple = collateralValue * zScore * simple_vol
yearly_VaR_simple = daily_VaR_simple * np.sqrt(365)

# VaR - GARCH
daily_VaR = collateralValue * zScore * volatility
yearly_VaR = daily_VaR * np.sqrt(365)

print("Daily VaR USD std-dev", daily_VaR_simple)
print("Yearly VaR USD std-dev", yearly_VaR_simple)
print("Daily VaR USD GARCH", daily_VaR.values[0])
print("Yearly VaR USD GARCH", yearly_VaR.values[0])
...
print("Daily VaR USD (t-distribution, GARCH volatility)",
      daily_VaR_t.values[0])
print("Yearly VaR USD (t-distribution, GARCH volatility)",
...
df['ES_hist'] = df['log_returns'][df['log_returns'] <=
                                  -df['VaR_hist'].iloc[0]].mean()
...
plt.plot(df['predicted_vol'],
         color='r',
         label='Predicted Volatility from GARCH(1, 1)')
plt.legend(loc='best')
